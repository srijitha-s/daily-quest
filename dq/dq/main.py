...
def push_quest(Quest, solution, Name):
    ref = db.reference("/QuestionBank") 
    ref.push({'solution': solution.strip(), "Name": Name.strip(), 
               'Quest': Quest.strip() , "used": 0})
...
def schedule_quest():
    ...
    scheduler.add_job(get_next_free_quest, trigger='cron', hour='00', minute='00', second="01")
    scheduler.start()
...
def evaluate_quest(user_data=None):
    ...
    return flask.redirect('/evaluate_past_quests')

@app.route("/add_quest", methods=["GET", "POST"])
@auth_required
def add_quest(user_data):
    if request.method == 'POST':
        push_quest(**request.form)
    print("Renderting add_quest.j2")
    return render_template("add_quest.j2", user_data=user_data)
